...
- Import Monzo CSV exports and Yonder PDF statements
...
# Upload Yonder PDF (account_id = 2)
curl -X POST http://localhost:8000/transactions/upload/2 \
  -F "file=@path/to/yonder-statement.pdf"
```
...
| POST | `/transactions/upload/{account_id}` | Upload statement |
...
| GET | `/reports/trends` | Spending trends over time |
